...
def search_attractions():
    ...
    attractions_list = [attraction for attraction in Attraction.query.filter(Attraction.city == city.id).all()] 
    ...
    attraction_ids = [attr.id for attr in attractions_list]
    #attraction_order = case(value=Attraction.id, whens={attr_id: index for index, attr_id in enumerate(attraction_ids)})
    
    #attractions_pagination = Attraction.query.filter(Attraction.id.in_(attractions_list)).order_by(attraction_order).paginate(page=page, per_page=per_page)
    
    #attraction_order_dict = {attr_id: index for index, attr_id in enumerate(attraction_ids)}

    # Query the attractions and order them based on their position in the list
    ...
    #base_attraction_ids = [(attraction.id) for attraction in Attraction.query.filter(Attraction.name.contains(search_text)).all()]
    #base_attraction_ids = [(attraction.id) for attraction in Attraction.query.filter(Attraction.city == city.id).all()] 
    print(f'attraction_ids: {attraction_ids}')
    ca, ag, ta = get_filter_ids(filter_priority_list, category, age_group, tag)
    print(ca)
    print(ag)
    print(ta)
    update_filter_form_choices(filter_form, attraction_ids, ca, ag, ta)
...
@attractions.route("/attractions/newnew", methods=["GET", "POST"])
def list_attractions():
    ...
    # Parse URL parameters for filtering
    #category = request.args.getlist("categories", type=int)
    #age_group = request.args.getlist("age_groups", type=int)
    #tag = request.args.getlist("tags", type=int)
    filter_priority = request.args.get('filterPriority')
    filter_priority_list = filter_priority.split(',') if filter_priority else []
    print(filter_priority_list)
    ...
    filter_form.categories.choices = [(category.id, category.name) for category in Category.query.filter(Category.id.in_(ca)).all()]
    filter_form.age_groups.choices = [(age_group.id, age_group.name) for age_group in AgeGroup.query.filter(AgeGroup.id.in_(ag)).all()]
    filter_form.tags.choices = [(tag.id, tag.name) for tag in Tag.query.filter(Tag.id.in_(ta)).all()]

    #filter_form.categories.choices = [(category.id, category.name) for category in Category.query.all()]
    #filter_form.age_groups.choices = [(age_group.id, age_group.name) for age_group in AgeGroup.query.all()]
    #filter_form.tags.choices = [(tag.id, tag.name) for tag in Tag.query.all()]
...
# Get all attractions.
@attractions.route("/attractions/old", methods=["GET", "POST"])
def get_attractions_old():
    if "selected_city" in session:
        selected_city = session["selected_city"]
    else:
        session["selected_city"] = 1
...
def update_filters(filter_priority, selected_categories, selected_age_groups, selected_tags):
    if filter_priority and filter_priority[0] == "categories":
        category_ids, primary_attraction_ids = get_primary_filter_ids(
            "categories", selected_categories
        )
