...
from app.models import Attraction, AttractionAgeGroup, AttractionCategory, AttractionTag, Category, AgeGroup, Tag, VisitedAttraction
from app import db
...
def get_attractions():
    ...
    age_groups_choices_query = db.session.query(AgeGroup.id, AgeGroup.name) \
        ...
        .order_by(AgeGroup.id).all()
            
    # Get IDs and Names of tags relevant to current attractions.
    tags_choices_query = db.session.query(Tag.id, Tag.name) \
    ...
    filter_form.categories.choices = [(category.id, category.name) for category in categories_choices_query]
...
def filter_attractions():
    ...
    if filter_priority and filter_priority[0] == 'categories':
        category_ids, primary_attraction_ids = get_primary_filter_ids('categories', selected_categories)
        ...
        if len(filter_priority) > 1:
            if filter_priority[1] == 'age_groups':
                tag_ids = get_tertiary_filter_ids('tags', selected_categories, selected_age_groups, selected_tags)
            elif filter_priority[1] == 'tags':
                age_group_ids = get_tertiary_filter_ids('age_groups', selected_categories, selected_age_groups, selected_tags)
    
    elif filter_priority and filter_priority[0] == 'age_groups':
        age_group_ids, primary_attraction_ids = get_primary_filter_ids('age_groups', selected_age_groups)
        ...
        if len(filter_priority) > 1:
            if filter_priority[1] == 'categories':
                ...
                category_ids = get_tertiary_filter_ids('categories', selected_categories, selected_age_groups, selected_tags)
        ...
        category_ids = db.session.query(AttractionCategory.category_id) \
            .distinct(AttractionCategory.category_id).all()
        age_group_ids = db.session.query(AttractionAgeGroup.age_group_id) \
            .distinct(AttractionAgeGroup.age_group_id).all()
        tag_ids = db.session.query(AttractionTag.tag_id) \
            .distinct(AttractionTag.tag_id).all()
                
    # Prepare AJAX response data.
    attractions_html = [render_template('attractions_items.html', attraction=attraction.to_dict()) for attraction in attractions]
...
def get_primary_filter_ids(filter_type, selected_filter_type_ids):
    ...
    primary_attraction_ids_query = db.session.query(Attraction.id) \
        .filter(attribute.any(relationship.in_(selected_filter_type_ids))).all()
    primary_attraction_ids = [attraction_id for (attraction_id,) in primary_attraction_ids_query]  

    primary_filter_ids = db.session.query(relationship) \
        .distinct(relationship).all()
...
def get_secondary_filter_ids(filter_type, primary_attraction_ids):
    ...
    secondary_filter_ids = db.session.query(relationship) \
...
def get_tertiary_filter_ids(filter_type, selected_categories, selected_age_groups, selected_tags):
    filters = []
    if filter_type == 'categories':
        model = AttractionCategory
        relationship = AttractionCategory.category_id
    ...
    secondary_attraction_ids_query = db.session.query(Attraction.id).filter(and_(*filters)).all()
    secondary_attraction_ids = [attraction_id for (attraction_id,) in secondary_attraction_ids_query]
            
    tertiary_filter_ids = db.session.query(relationship) \
